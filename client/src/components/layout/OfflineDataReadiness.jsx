...
export default function OfflineDataReadiness({ readiness, diagnostics, retry, failureMessage = "" }) {
  ...
  if (!readyNotice || readyAcknowledged) return <button type="button" aria-label={`Offline data status: ${title}`} onClick={() => setReadyNotice(true)} style={{ ...buttonStyle, position: "fixed", left: "50%", bottom: "max(16px, env(safe-area-inset-bottom))", transform: "translateX(-50%)", zIndex: 1200, boxShadow: "0 8px 20px rgba(31,64,96,.16)" }}>{title}</button>;
