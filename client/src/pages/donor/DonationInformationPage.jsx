          ...
          <div style={styles.heroAsideCard}>
            ...
            <p style={styles.heroAsideText}>
              {hasActiveDisasters
                ...
                : "No active disasters are currently open, but the portal remains available for public monitoring."}
            </p>
          </div>
...
const CriticallyNeededItemsSection = ({ activeDisasters }) => {
  const items = activeDisasters
    .flatMap((disaster) =>
      (disaster.urgentNeeds || []).map((item) => ({
        ...item,
        disasterName: disaster.eventName,
      })),
    )
    .sort((left, right) => {
      if (left.level !== right.level) {
      ...
      <div style={styles.sectionHeader}>
        <div>
          ...
          <p style={styles.sectionText}>
            Priority donation needs based on the current active disaster
            situation across Malvar.
          </p>
        </div>
      </div>
      ...
      <div style={styles.neededGrid}>
        {items.length === 0 ? (
          <p style={styles.inlineEmptyText}>
            No prioritized donation items are available yet from the current
            active disaster records.
          </p>
        ) : (
          items.map((item) => {
            const palette = getNeedPalette(item.level);
            ...
            return (
              ...
              >
                ...
                <h3 style={styles.neededTitle}>{item.name}</h3>
...
const DropOffLocationSection = () => {
  return (
    <section style={styles.dropOffSection}>
      <h2 style={styles.sectionTitle}>Drop-off Location</h2>
      <p style={styles.dropOffSubtitle}>
        Donations may be coordinated and delivered through the LGU handling
        point below.
      </p>
      <div style={styles.dropOffCard}>
        <FiMapPin size={42} color={COLORS.primary} />
        <div>
          <p style={styles.dropOffLocationName}>Malvar Municipal Hall</p>
          <p style={styles.dropOffLocationText}>{LGU_CONTACT.address}</p>
        </div>
      </div>
    </section>
  );
    ...
    <footer style={styles.footer}>
      <div style={styles.footerTop}>
        <div>
          ...
          <p style={styles.footerSubtitle}>
            ...
            please contact the Malvar LGU through the channels below.
          </p>
        </div>
      </div>
      ...
      <div style={styles.footerGrid}>
        <div style={styles.footerCard}>
          ...
          <div>
            <p style={styles.footerLabel}>Contact Number</p>
            <p style={styles.footerValue}>{LGU_CONTACT.phone}</p>
          </div>
        </div>
        ...
        <div style={styles.footerCard}>
          ...
          <div>
            <p style={styles.footerLabel}>Address</p>
            <p style={styles.footerValue}>{LGU_CONTACT.address}</p>
          </div>
        </div>
        ...
        <div style={styles.footerCard}>
          ...
          <div>
            ...
            <p style={styles.footerValue}>{LGU_CONTACT.email}</p>
          </div>
        </div>
      </div>
    </footer>
      ...
      <div style={styles.pageInner}>
        ...
        <div style={styles.contentStack}>
          {!isLoading && !errorMessage && hasActiveDisasters ? (
            <CriticallyNeededItemsSection activeDisasters={activeDisasters} />
          ) : null}
          ...
          {!isLoading && !errorMessage && hasActiveDisasters
            ? activeDisasters.map((disaster) => (
                <section key={disaster.id} style={styles.disasterCard}>
                  ...
                  <div
                    className="donor-portal-content-grid"
                    ...
                    <div style={styles.subSection}>
                      ...
                      <div style={styles.urgentList}>
                        {disaster.urgentNeeds.length === 0 ? (
                          <p style={styles.inlineEmptyText}>
                            No event-linked inventory summary is available yet
                            for this active disaster.
                          </p>
                        ) : (
                          disaster.urgentNeeds.map((item) => {
                    ...
                    <div style={styles.subSection}>
                      ...
                      <div style={styles.areaList}>
                        {disaster.areaBreakdown.length === 0 ? (
                          <p style={styles.inlineEmptyText}>
                            No barangay-level impact summary is available yet
                            for this active disaster.
                          </p>
                        ) : (
