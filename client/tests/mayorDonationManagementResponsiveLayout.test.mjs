...
test("Mayor donation management page exposes scoped responsive hooks", async () => {
  ...
  assert.match(
    cssSource,
    /@media \(max-width: 480px\)[\s\S]*?\.mayor-donation-management-summary-grid \{[\s\S]*?grid-template-columns: repeat\(2, minmax\(0, 1fr\)\) !important;/,
  );
  assert.match(
    cssSource,
    /@media \(max-width: 768px\)[\s\S]*?\.mayor-donation-management-summary-grid[\s\S]*?grid-template-columns: repeat\(2, minmax\(0, 1fr\)\) !important;/,
  );
  assert.match(
    cssSource,
...
test("Mayor donation management records and details keep table overflow local", async () => {
  ...
  assert.match(
    donationsSource,
    /donationFormHeaderCell: \{[\s\S]*?width: "130px"[\s\S]*?minWidth: "130px"/,
  );
  assert.match(
    donationsSource,
    /donationFormBodyCell: \{[\s\S]*?width: "130px"[\s\S]*?minWidth: "130px"/,
  );
  assert.match(donationsSource, /itemsHeaderCell:[\s\S]*?width: "21%"[\s\S]*?minWidth: "210px"/);
  assert.match(
    donationsSource,
    /donationTypeLabel[\s\S]*?donationFormBodyCell/,
  );
  assert.match(
    donationsSource,
    /quantityBodyCell[\s\S]*?itemDetails\.quantityLines/,
  );
  assert.match(
    donationsSource,
    /dateBodyCell[\s\S]*?formatDonationDateTime/,
  );
  ...
  assert.match(
    transparencySource,
    /disasterEventBodyCell: \{[\s\S]*?textAlign: "left"/,
  );
  assert.match(
    transparencySource,
    /disasterEventHeaderCell: \{[\s\S]*?textAlign: "left"/,
  );
  assert.match(
    transparencySource,
    /disasterEventHeaderCell[\s\S]*?Disaster Event/,
  );
