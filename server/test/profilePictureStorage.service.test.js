...
test("successful profile picture upload does not emit a failure diagnostic", async () => {
...
test("Storage upload failure logs one sanitized failure and keeps the generic error", async () => {
  await withStorageConfig(async () => {
    uploadCalls.length = 0;
...
test("validation failure before Storage does not emit a failure diagnostic", async () => {
  await withStorageConfig(async () => {
    uploadCalls.length = 0;
    const entries = await captureStorageLogs(() =>
      assert.rejects(
