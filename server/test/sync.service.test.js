...
const supplierRepositoryPath = require.resolve("../src/repositories/supplier.repository");
...
test("legacy Supplier create and update queue entries remain readable and syncable", async () => {
  const calls = [];

  await withStubbedSyncService(
    {
      [syncRepositoryPath]: createBaseSyncRepositoryStub(),
      [supplierRepositoryPath]: {
      ...
      [supplierServicePath]: {
        createSupplier: async (payload) => {
          ...
          return { id: "supplier-created", ...payload };
        },
        updateSupplier: async (id, payload) => {
          calls.push({ action: "update", id, payload });
          return { id, ...payload };
        },
      },
      [systemLogPath]: {
    ...
    async ({ processSyncEntries }) => {
      const results = await processSyncEntries({
        ...
        entries: [
          {
            client_sync_id: "legacy-supplier-create-1",
            action_key: "SUPPLIER_CREATE",
            entity_type: "SUPPLIER",
            entity_local_id: "legacy-supplier-local-1",
            client_timestamp: "2026-08-08T01:00:00.000Z",
            payload: {
              name: "New legacy supplier",
              contact_person: "A. Contact",
              has_moa: false,
            },
          },
          {
            client_sync_id: "legacy-supplier-update-1",
            action_key: "SUPPLIER_UPDATE",
            entity_type: "SUPPLIER",
            entity_server_id: "supplier-existing",
            entity_local_id: "supplier-existing",
            client_timestamp: "2026-08-08T01:01:00.000Z",
            payload: {
              name: "Updated legacy supplier",
              contact_person: null,
              has_moa: true,
            },
          },
...
test("legacy inventory batch payloads carrying supplier_id still sync through the existing batch handler", async () => {
  ...
  await withStubbedSyncService(
    ...
    async ({ processSyncEntries }) => {
      ...
      assert.equal(result.sync_status, "SYNCED");
      assert.equal(
        capturedPayload.supplier_id,
        "22222222-2222-4222-8222-222222222222",
      );
      assert.equal(capturedPayload.legacySupplierCompatibility, true);
      assert.equal(capturedPayload.created_by, baseAuth.userId);
      assert.equal(capturedPayload.received_at, "2026-08-08T01:02:00.000Z");
    },
  );
});
